//////////////////////////////////////////////////////////////////////////////////////////
// pFactorDegrees added, Dec 2015 by S.Pauli
...
function pfactordegrees(p, polynomial)
  _,_,_,EF := localmontesmatrix(polynomial,p:diag:="None");
  return [ef[1]*ef[2]:ef in EF];
end function;

intrinsic pFactorDegrees(p::RngIntElt,polynomial::RngUPolElt)->SeqEnum
{The degrees of the irreducible factors of the square free polynomial over the p-adic integers}
  fact := Factorization(polynomial);
  return &cat[pfactordegrees(p,f[1]):f in fact];
end intrinsic;
...
//"lifa[i]",lifa[i],Coefficients(lifa[i]);
//"Pol",Pol;
//"K",K;
//(Coefficients(lifa[i] mod Pol));
//(Coefficients(lifa[i] mod Pol) cat [0: j in [Degree(lifa[i] mod Pol)+2..Degree(K)]]);
                primeid`generator:=K!(Coefficients(lifa[i] mod Pol) cat [0: j in [Degree(lifa[i] mod Pol)+2..Degree(K)]]);
                Append(~idealfactors,primeid);
