      ...
      function residual_polynomial_classes_sub(phi,with_trans)
        ...
        for delta in RK do
          if delta ne 0 then
            ...
            if with_trans then
//"classes",delta," : ",ResidualPolynomial(phidelta),Log(KtoRK(ConstantCoefficient(phidelta)/pi));
...
//"Base",base,"delta",delta; rpp := residual_polynomial_phis(base,delta); "res polys", [r[1]: r in rpp]; "log",[Log(KtoFq(ConstantCoefficient(a[2])/Prime(K))) : a in rpp];
...
//"As",As;
       ...
       //phis := [a[2]: a in As | a[1] eq As[1][1]];
       philogs := [<Log(KtoFq(ConstantCoefficient(a[2])/piK)),a[2]>: a in As | a[1] eq As[1][1]];
//"philogs",philogs;
